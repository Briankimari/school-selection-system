      ...
      <div className="mt-5 ">
        {bmiData.map((item, index) => (
          <div key={index} className="flex items-center leading-8 gap-5 border-b-1 border-color p-3">
            <Avatar className="rounded-full h-10 w-10" src={item.firstName} alt={item.message} />
            <div>
              <p className="font-semibold dark:text-gray-200">{item.firstName} {item.surName}</p>
              <p className="text-gray-500 text-sm text-left dark:text-gray-400">your account have been activated with 7 free school check out </p>
            </div>
          </div>
        ))}
        <div className="mt-5">
